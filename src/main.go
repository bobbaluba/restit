...
func main() {
	...
	renderWindow := sf.NewRenderWindow(sf.VideoMode{gameWidth, gameHeight, 32}, "Pong (GoSFML2)", sf.StyleDefault, nil)
	...
	for renderWindow.IsOpen() {
		...
		case <-ticker.C:
			...
			if isPlaying {
				...
				if (rightPaddleSpeed < 0 && rightPaddle.GetPosition().Y-paddleSize.Y/2 > 5) || (rightPaddleSpeed > 0 && 
rightPaddle.GetPosition().Y+paddleSize.Y/2 < float32(gameHeight)-5) {
